...
for i = 1:num_var
    ...
    W1 = source_sink_swirl(i*5i,0,-10:1:10);
    W2 = source_sink_swirl(i*1,0,-10:1:10);
    W3 = source_sink_swirl(i*-1,0,-10:1:10);
    W4 = doublet(.1*i,0);
    ...
    if any(i == [1,10])
        switch i, case 1, n = 1; str = 'low'; case 10, n = 2; str = 'high'; end
        figure(3)
        h(n) = subplot(1,2,n);
    ...
    [Cvv(i,:),R] = spatial_correlation_function(V,centroids,nbins,'off','on');
end
...
[W0,X,Y] = power_law(1,pi/2,1,x);
W1 = source_sink_swirl(1i,zA,x);
W2 = doublet(10,zA,x);
...
V = grid2list(flat(V(:,:,1)),flat(V(:,:,2)));
centroids = grid2list(X(:),Y(:));
[Cvv,R] = spatial_correlation_function(V,centroids,25,'off');
figure,plot(R,Cvv);