...
cluster_pulse_density(1,:) = nanmean(num_near(:, ...
    [pulseOI([pulseOI.cluster_label]==1).pulseID]),2);
cluster_pulse_density(2,:) = nanmean(num_near(:,...
    [pulseOI([pulseOI.cluster_label]==2).pulseID]),2);
cluster_pulse_density(3,:) = nanmean(num_near(:,...
    [pulseOI([pulseOI.cluster_label]==3).pulseID]),2);
cluster_pulse_density(4,:) = nanmean(num_near(:,...
    [pulseOI([pulseOI.cluster_label]==4).pulseID]),2);
cluster_pulse_density(5,:) = nanmean(num_near(:,...
    [pulseOI([pulseOI.cluster_label]==5).pulseID]),2);

cluster_std(1,:) = nanstd(num_near(:, ...
    [pulse([pulse.cluster_label]==1).pulseID]),[],2);
cluster_std(2,:) = nanstd(num_near(:,...
    [pulse([pulse.cluster_label]==2).pulseID]),[],2);
cluster_std(3,:) = nanstd(num_near(:,...
    [pulse([pulse.cluster_label]==3).pulseID]),[],2);
cluster_std(4,:) = nanstd(num_near(:,...
    [pulse([pulse.cluster_label]==4).pulseID]),[],2);
cluster_std(5,:) = nanstd(num_near(:,...
    [pulse([pulse.cluster_label] ==5).pulseID]),[],2);
...
% legend('Cluster 1','Cluster 2','Cluster 3','Cluster 4','Cluster 5','WT Average','twist average');
