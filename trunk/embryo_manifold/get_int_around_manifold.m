...
layers_around = permute(layers_around,[3 1 2]);
...
if strcmpi(interp,'on')
    ...
    for i = 1:X
        for j = 1:Y
            alpha = manifold_layers(i,j,n_levels+1) - manifold_floor(i,j,n_levels+1);
            int_found(i,j,:) = ...
                (1-alpha)*img_stack(i,j,manifold_floor(i,j,:)) ...
                + alpha*img_stack(i,j,manifold_ceil(i,j,:));
        end
    end
else
    ...
    for i = 1:X
        for j = 1:Y
            int_found(i,j,:) = img_stack(i,j,manifold_layers(i,j,:));
        end
    end
