function output = EDGE_processing(handle)
%POST_EDGE
%
...
if ~isempty(areas) && ~isempty(myosins)
    ...
    save([output_dir '/correlations/whole_embryo/myosinrate_constrictionrate'],'correlations');
...
if ~isempty(foc_meas) && ~isempty(neighb_meas) && ~isempty(neighborID)
    ...
    % Save data
    save([output_dir '/neighbor_focus/whole_embryo/neighbor_angles'],'angles_mat');
    save([output_dir '/neighbor_focus/whole_embryo/' handle.neighbor_focus.neighbor_angles.angle_data],'angle_data');
...
if ~isempty(myosins)
    ...
    if handle.peak_gauss.display.allresponse
        % Make movies
    ...
    if handle.peak_gauss.display.positive
    ...
    if handle.peak_gauss.display.negative
    ...
    save([output_dir '/peak_gauss/peak_locations'],'peaks');
...
if ~isempty(myosins)
    ...
    if handle.peak_consecutive.display.allresponse
        % Make movies
    ...
    if handle.peak_consecutive.display.positive
    ...
    if handle.peak_consecutive.display.negative
        % All decreasing (expanding) cells
        decreasing = nan(size(response_consec));
        decreasing(response_consec < 0) = response_consec(response_consec < 0);
        h.todraw = 1:num_cells;
        h.m = decreasing;
    ...
    save([output_dir '/peak_consecutive_' num2str(count_threshold) '/peak_locations'],'roi_consec');
    
    output_h.peak_consecutive = 1;
    
end

%% Coronal correlations

if ~isempty(areas) && ~isempty(neighborID)
    
    mkdir([output_dir '/corona/']);
    coronal_areas = get_corona_area(areas,neighborID);
    coronal_areas(:,handle.corona.ignore) = nan;
    ...
    wt = handle.corona.wt;
    [x,y] = meshgrid(-wt:wt,1:num_cells);
    area_corona_corr = nanxcorr(areas_rate,coronal_areas_rate,wt);
    
    figure(1);
    showsub(@pcolor,{x,y,area_corona_corr},'Dynamic correlation between corona constriction rate and cell constriction rate','axis equal tight;colorbar;',...
         @errorbar,{-wt:wt,nanmean(area_corona_corr),nanstd(area_corona_corr)},'Average correlation between corona constriction rate and cell constriction rate','xlabel(''Time sfhit (frames)''),ylabel(''Correlation'')' ...
         );
    
    saveas(gcf,[output_dir '/corona/correlation_corona_focal']);
    
%     figure(1),pcolor(x,y,area_corona_corr);
%     axis equal tight;colorbar;
%     title('Dynamic correlation between corona constriction rate and cell constriction rate');
%     xlabel('Time sfhit (frames)')
%     ylabel('Cells')
%     
%     saveas(gcf,[output_dir '/corona/correlation_corona_focal']);
%     
%     errorbar(-wt:wt,nanmean(area_corona_corr),nanstd(area_corona_corr));
%     title('Average correlation between corona constriction rate and cell constriction rate');
%     xlabel('Time sfhit (frames)')
%     ylabel('Correlation')
%     
%     saveas(gcf,[output_dir '/corona/correlation_corona_focal_mean']);
    
    % early frames
    [x,y] = meshgrid(-wt:wt,1:num_cells);
    area_corona_corr_early = nanxcorr(areas_rate(1:floor(num_frames/2),:),coronal_areas_rate(1:floor(num_frames/2),:),wt);
    
    figure(1);
    showsub(@pcolor,{x,y,area_corona_corr_early},'Dynamic correlation between EARLY corona constriction rate and cell constriction rate','axis equal tight;colorbar;',...
         @errorbar,{-wt:wt,nanmean(area_corona_corr_early),nanstd(area_corona_corr_early)},'Average correlation between EARLY corona constriction rate and cell constriction rate','xlabel(''Time sfhit (frames)''),ylabel(''Correlation'')' ...
         );
    
    saveas(gcf,[output_dir '/corona/correlation_corona_focal_early']);
    
%     figure(1),pcolor(x,y,area_corona_corr_early);
%     axis equal tight;colorbar;
%     title('Dynamic correlation between EARLY corona constriction rate and cell constriction rate');
%     xlabel('Time sfhit (frames)')
%     ylabel('Cells')
%     
%     saveas(gcf,[output_dir '/corona/correlation_corona_focal_early']);
%     
%     errorbar(-wt:wt,nanmean(area_corona_corr_early),nanstd(area_corona_corr_early));
%     title('Average correlation between EARLY corona constriction rate and cell constriction rate');
%     xlabel('Time sfhit (frames)')
%     ylabel('Correlation')
%     
%     saveas(gcf,[output_dir '/corona/correlation_corona_focal_mean_early']);
    
    % late frames
    [x,y] = meshgrid(-wt:wt,1:num_cells);
    area_corona_corr_late = nanxcorr(areas_rate(ceil(num_frames/2:end),:),coronal_areas_rate(ceil(num_frames/2:end),:),wt);
    
    figure(1);
    showsub(@pcolor,{x,y,area_corona_corr_late},'Dynamic correlation between LATE corona constriction rate and cell constriction rate','axis equal tight;colorbar;',...
         @errorbar,{-wt:wt,nanmean(area_corona_corr_late),nanstd(area_corona_corr_late)},'Average correlation between LATE corona constriction rate and cell constriction rate','xlabel(''Time sfhit (frames)''),ylabel(''Correlation'')' ...
         );
    
    saveas(gcf,[output_dir '/corona/correlation_corona_focal_late']);
    
    save([output_dir '/corona/correlation_corona_focal'],'area_corona_corr');
    save([output_dir '/corona/correlation_corona_focal_early'],'area_corona_corr_early');
    save([output_dir '/corona/correlation_corona_focal_late'],'area_corona_corr_late');
...
if output_h.corona
    this.name = 'correlation between coronal constriction rate and area rate';
    this.data = area_corona_corr;
    if first
        output = this;
        first = 0;
    else
    ...
    this.name = 'early correlation between coronal constriction rate and area rate';
    this.data = area_corona_corr_early;
    output = [output this];
    this.name = 'late correlation between coronal constriction rate and area rate';
    this.data = area_corona_corr_late;
    output = [output this];
end
varargout{1} = output;
