...
[X,Y] = meshgrid(-10:11,1:num_peaks);
...
showsub_vert(...
    ...
    @plot,{-10:11,data2cluster(labels==1,:)},'','',...
    @errorbar,{-10:11,nanmean(data2cluster(labels==1,:)),nanstd(data2cluster(labels==1,:),[])},'','xlabel(''Aligned time'')',...
    @pcolor,{X(1:numel(labels(labels==2)),:),Y(1:numel(labels(labels==2)),:),data2cluster(labels==2,:)},...
    '2','shading flat;caxis([-20 20]);colorbar',...
    @plot,{-10:11,data2cluster(labels==2,:)},'','',...
    @errorbar,{-10:11,nanmean(data2cluster(labels==2,:)),nanstd(data2cluster(labels==2,:),[])},'','xlabel(''Aligned time'')',...
    @pcolor,{X(1:numel(labels(labels==3)),:),Y(1:numel(labels(labels==3)),:),data2cluster(labels==3,:)},...
    '3','shading flat;caxis([-20 20]);colorbar',...
    @plot,{-10:11,data2cluster(labels==3,:)},'','',...
    @errorbar,{-10:11,nanmean(data2cluster(labels==3,:)),nanstd(data2cluster(labels==3,:),[])},'','xlabel(''Aligned time'')',...
    3);
