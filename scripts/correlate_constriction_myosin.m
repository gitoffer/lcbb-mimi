...
figure,showsublink(@imagesc,{t,[1 sum(num_cells)],areas_sm'},'Cell areas','colorbar;', ...
    @imagesc,{[1 num_frames],[1 sum(num_cells)],myosins_sm'},'Myosin intensity (clipped)','colorbar' ...
    );

figure,showsublink(@imagesc,{t,[1 sum(num_cells)],areas_rate'},'Constriciton rate','colorbar;', ...
    @imagesc,{t,[1 sum(num_cells)],myosins_rate'},'Myosin rate','colorbar' ...
    );

figure
showsub(@imagesc,{([-wt wt])*input(1).dt,[1 num_cells(1)], correlations(c==1,:)}, ...
    'Cross-correlation per cell. Embryo #1','colorbar', ...
    @errorbar,{(-wt:wt)*8,nanmean(correlations(c==1,:)),nanstd(correlations(c==1,:))},...
    'Mean cross-correlation, embryo #1','axis on',...
    @imagesc,{([-wt wt])*input(2).dt,[1 num_cells(2)], correlations(c==2,:)}, ...
    'Cross-correlation per cell. Embryo #2','colorbar', ...
    @errorbar,{(-wt:wt)*8,nanmean(correlations(c==2,:)),nanstd(correlations(c==2,:))},...
    'Mean cross-correlation, embryo #2','axis on' ...
    );
