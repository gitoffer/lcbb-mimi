...
showsub(@imagesc,{([-wt wt])*input(1).dt,[1 num_cells(1)], correlations([IDs.which]==1,:)}, ...
    ...
    @errorbar,{(-wt:wt)*8,nanmean(correlations([IDs.which]==1,:)),nanstd(correlations([IDs.which]==1,:))},...
    'Mean cross-correlation, embryo #1','axis on',...
    @imagesc,{([-wt wt])*input(2).dt,[1 num_cells(2)], correlations([IDs.which]==2,:)}, ...
    'Cross-correlation per cell. Embryo #2','colorbar', ...
    @errorbar,{(-wt:wt)*8,nanmean(correlations([IDs.which]==2,:)),nanstd(correlations([IDs.which]==2,:))},...
